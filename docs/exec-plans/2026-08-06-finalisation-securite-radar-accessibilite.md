...
5. **Diffusion SSE.** Pool du planificateur à 4 et exécuteur dédié à un seul thread, file bornée à 64 abandonnant le plus ancien instantané. Une écriture bloquée ne peut plus figer le balayage ni un thread de requête. Résidu alors documenté, puis supprimé : voir « Reconnexion SSE et isolation des diffusions par flux » ci-dessous.
...
## Reconnexion SSE et isolation des diffusions par flux

L'étude des suites possibles au correctif précédent a mis au jour un défaut plus grave que celui qu'elles visaient.

**Reconnexion.** `RadarApiService.events()` appelait `source.close()` dans `onerror`, ce qui annule définitivement la reconnexion automatique d'`EventSource`. La page basculait alors sur un sondage de dix secondes pour le reste de la session. Toute coupure, même d'une seconde, privait donc ce joueur du temps réel — et, `SSE_TIMEOUT_MS` valant une heure, **tout joueur restant plus d'une heure sur Radar tombait en mode dégradé** sans qu'aucun client lent ne soit impliqué. Le flux émet désormais un état (`connected`, `reconnecting`, `snapshot`) : une erreur dont l'état est `CONNECTING` laisse le navigateur réessayer, seul `CLOSED` reste définitif. Le sondage de secours devient transitoire, s'arrête au premier événement reçu, et absorbe ses propres échecs — un tirage en erreur emportait auparavant la souscription entière.

**Isolation.** La diffusion partait d'un exécuteur partagé à un seul thread qui parcourait tous les émetteurs : un client bloqué retardait les instantanés de tous les autres jusqu'à la fin de sa connexion, résidu alors documenté comme assumé. Chaque flux possède maintenant sa file bornée à 16 événements et son unique écrivain, un thread virtuel ; diffuser se limite à un dépôt non bloquant. Un client saturé est lâché plutôt que suivi — le correctif de reconnexion le fait revenir avec un état frais — et l'émetteur n'est jamais terminé depuis le diffuseur, `complete()` pouvant lui-même attendre le verrou d'écriture. `RadarDeliveryExecutor` disparaît : plus aucun `Executor` dans le paquet, donc plus de support au piège d'autoconfiguration décrit ci-dessus, dont le garde-fou reste en place.

**Point d'attention sur les tests.** La première version de `RadarEventBroadcasterTests` simulait les émetteurs avec Mockito : un émetteur dont l'écriture reste bloquée bloque aussi les autres simulations du même cadre, ce qui faisait échouer le test d'isolation alors que le code de production était correct. Les émetteurs de test sont donc écrits à la main, ce qui permet en outre d'assertionner le contenu réellement écrit. Une seconde leçon en est sortie : la file borne aussi les rafales, l'écrivain devant être ordonnancé pour la vider — quarante diffusions en quelques microsecondes lâchent même un client sain, ce qui ne correspond à aucun rythme réel (quelques diffusions par seconde) mais devait être compris avant de figer la capacité.

Validations : 124 tests backend, 102 tests frontend, lint et build frontend propres.

## Corrections issues de l'audit

L'audit de ce travail n'a relevé aucun blocage, mais un défaut sérieux introduit par la correction elle-même, trois défauts dans le code neuf et deux modes dégradés sans issue. Tous sont corrigés.

**Un signal ne peut pas porter deux pannes.** `streamError` était levé aussi bien par une réception défaillante que par une publication en échec, et la nouvelle règle « tout événement reçu referme le bandeau » le rendait fugace : un aventurier invisible aux autres lisait « Le lien direct vacille », message effacé quelques secondes plus tard par un instantané. `publishError` est désormais distinct, avec son propre bandeau, refermé par la seule publication réussie.

**Trois défauts dans `RadarStream`.** L'écrivain était affecté après son démarrage : un `close()` concurrent pouvait n'interrompre personne et laisser un thread garé pour toujours sur un flux déjà retiré du registre. Il est maintenant publié avant d'être démarré, pour qu'une fermeture simultanée le trouve, et le démarrage se termine par un contrôle du drapeau qui couvre le cas inverse, celui d'une fermeture antérieure à cette publication. Ce contrôle a été ajouté après coup : la première version s'appuyait sur l'interruption d'un thread non encore démarré, effective sur Temurin 25 — deux cents essais — mais que le javadoc de `Thread.interrupt()` déclare sans effet garanti. Interrompre un thread désormais vivant, lui, est pleinement défini. Une erreur inattendue à l'écriture, un échec de sérialisation par exemple, tuait par ailleurs le thread avec une trace brute : elle est désormais journalisée en `WARN` et traitée comme un échec d'écriture. Enfin le commentaire de `close()` affirmait à tort que l'interruption était réservée à l'arrêt, alors que les rappels du conteneur la déclenchent en exploitation normale ; ce qui la rend sans risque est que ces rappels n'arrivent que sur une réponse déjà terminée ou mourante.

**Deux modes dégradés qui ne se refermaient jamais.** Une coupure définitive du flux laissait la session entière en sondage — exactement le défaut que la reconnexion native venait de supprimer, revenu par une autre porte : le flux est réarmé au bout d'une minute. Un premier instantané en échec ne relançait rien : il rejoint désormais la réception dégradée et son sondage. `ensureMap()` a par ailleurs reçu le garde qui lui manquait : l'`await import('leaflet')` le rendait réentrant, et deux relevés rapprochés produisaient un rejet non géré. Ce dernier point n'a pas de test unitaire, jsdom n'hébergeant pas Leaflet — ce pour quoi la suite simule systématiquement `ensureMap`.

**Piège de comptage.** Le total annoncé plus haut avait d'abord été lu comme la somme des rapports Surefire du répertoire `target`, que Maven n'efface pas : un fichier laissé par une classe supprimée au commit `e0279bd` gonflait le total de 124 à 128. Le seul total qui fasse foi est la ligne « Tests run » imprimée par Maven.

Validations : 125 tests backend, 106 tests frontend, lint et build frontend propres, `RadarEventBroadcasterTests` rejoué plusieurs fois, dont une exécution avec un unique thread porteur (`jdk.virtualThreadScheduler.parallelism=1`) qui prouve que l'isolation ne dépend d'aucune taille de pool.
