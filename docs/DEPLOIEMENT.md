...
10. Une session Access expirée pendant l'utilisation de la SPA n'emmène jamais l'utilisateur hors de sa page : elle affiche un bandeau, et lui seul décide de se reconnecter. Aucune boucle n'est possible, rien ne navigue sans un clic. Le bouton recharge la page en portant le marqueur `ngsw-bypass` : privée de session valide, la navigation reçoit de Cloudflare sa redirection vers l'authentification, puis revient à l'adresse d'origine. Aucune déconnexion n'est demandée au passage — elle faisait atterrir sur `/?__cf_access_message=logged_out`, adresse choisie par Cloudflare que l'application ne peut pas marquer, donc interceptée par le service worker, et dont le premier chargement échouait en `ERR_FAILED`. **À vérifier depuis l'accueil ou `/carnet`, pas seulement depuis `/radar` :** ces pages sont servies depuis le cache du service worker, et c'est précisément là que la reconnexion échouait avant le marqueur.
