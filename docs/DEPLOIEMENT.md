...
10. Une session Access expirée pendant l'utilisation de la SPA ne provoque aucune navigation automatique : l'en-tête remplace le menu de profil par un lien « Se reconnecter », et rien ne bouge tant qu'il n'est pas suivi.
...
14. Depuis une page publique servie par le cache du service worker — `/`, `/map`, `/notebook` —, le lien « Se reconnecter » quitte réellement le navigateur : l'onglet Réseau montre un document `/reconnexion` avec une taille en octets, jamais `(from ServiceWorker)`, une `302`, et une requête vers `*.cloudflareaccess.com`. L'utilisateur revient ensuite sur la page qu'il consultait, pas sur l'accueil.
15. Sans réseau et session expirée, l'en-tête affiche « Hors ligne » et n'offre aucun lien de reprise. Sans réseau mais session valide, le menu de profil reste affiché tel quel.
16. Ce relevé n'a de valeur que si le navigateur exécute bien le dernier bundle. Hors session Access, le service worker ne peut plus récupérer `ngsw.json` — Cloudflare répond une redirection, que la requête suit vers une autre origine avant d'échouer — donc l'application reste figée sur sa version précédente. Comparer le hash de `https://.../ngsw/state` avant et après déploiement, ou désinscrire le service worker session valide, avant de conclure quoi que ce soit.
